...
export default function About() {
  ...
  // Function to read the text aloud with real-time highlighting
  const readTextAloud = () => {
    const speech = new SpeechSynthesisUtterance(text);
    speech.lang = 'en-US';

    // Event fired when reaching a word boundary
    speech.onboundary = (event) => {
      ...
      for (let i = 0; i < words.length; i++) {
        countChars += words[i].length + 1; // +1 for the space
    ...
    // Reset highlighting after reading is complete
    speech.onend = () => {
  ...
  return (
    <div
      ...
      onClick={readTextAloud} // Trigger readTextAloud on click
      style={{ cursor: 'pointer' }} // Change cursor to pointer for better UX
    >
      <div className="z-10">
        <h1 className="text-[20px] flex items-center gap-3 text-[#0CC880] font-semibold">About Me <span><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
